...
export default async function BallotPage() {
    ...
    return (
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          Belum ada kandidat aktif saat ini.
        </p>
      </div>
    );
  }
  ...
  const gridClasses =
    candidates.length === 1
      ...
      : candidates.length === 2
      ? "grid-cols-2 max-w-md sm:max-w-xl mx-auto"
      : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 max-w-6xl mx-auto";

  return (
    <form
      action={castVote}
      className="w-full pb-[calc(6rem+env(safe-area-inset-bottom))]"
    >
        ...
        {candidates.map((c) => (
          <div key={c.id} className="relative w-full">
            ...
            <label
              ...
              className="flex flex-col h-full rounded-xl border bg-card shadow-sm overflow-hidden hover:bg-accent/50 peer-checked:border-primary peer-checked:ring-1 peer-checked:ring-primary transition-all cursor-pointer scroll-mb-28"
            >
              <div className="aspect-[3/4] sm:aspect-[4/5] relative w-full bg-muted">
                {c.fotoUrl ? (
                  <Image
                    ...
                    sizes="(min-width: 1024px) 320px, (min-width: 640px) 288px, 45vw"
                  />
                ) : (
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-t from-background to-background/80 backdrop-blur-sm border-t">
